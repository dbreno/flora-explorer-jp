...
import { plants, ranking, missoes, assets, type Plant } from "@/lib/flora-data";
  ...
  return (
    <div className="min-h-screen bg-cream sm:bg-sage/20">
      ...
      <div className="mx-auto hidden max-w-5xl items-center justify-between px-6 py-4 sm:flex">
        ...
        <span className="font-display text-lg text-moss">App — Visão Aluno</span>
        <Link
          to="/web"
  ...
  return (
    <div className="space-y-4 px-5 pt-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="rounded-2xl bg-moss px-3 py-2 text-right text-moss-foreground">
          <p className="text-[10px] uppercase tracking-wider opacity-80">Nível</p>
          <p className="font-display text-2xl leading-none">4</p>
        </div>
      </div>

      {/* XP bar */}
      <div className="rounded-2xl border border-moss/15 bg-card p-4">
        <div className="flex items-center justify-between text-sm">
          <span className="flex items-center gap-1.5 font-medium text-moss">
            <Sparkles className="h-4 w-4 text-xp" /> XP da semana
          </span>
          <span className="font-medium text-moss">320 / 500</span>
        </div>
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Faltam <span className="font-medium text-moss">180 XP</span> para o nível 5
        </p>
      </div>
        ...
        <div className="space-y-2">
          {missoes.map((m) => (
            <button
              key={m.id}
              onClick={onCapture}
              className="block w-full rounded-xl border border-border/60 bg-card p-3 text-left transition hover:border-moss/40"
            >
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-medium leading-snug text-foreground">{m.title}</p>
                <span className="shrink-0 rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium text-moss">
                  {m.deadline}
                </span>
              </div>
      ...
      {/* Alerta invasora */}
      <div className="rounded-2xl border border-invasive/30 bg-invasive/8 p-3">
        <div className="flex items-start gap-2.5">
          <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-invasive" />
          <div className="min-w-0">
            <p className="text-sm font-semibold text-invasive">+50 XP Bônus essa semana</p>
  ...
  return (
    <div className="space-y-4 px-5 pt-6">
      <div>
        ...
        <h1 className="font-display text-3xl text-moss">Sua coleção</h1>
      </div>
...
function MyPlantsScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-4 px-5 pt-6">
      ...
      <h1 className="font-display text-3xl text-moss">Minhas Plantas</h1>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-card border border-border p-3 text-center">
      <div className="font-display text-2xl text-moss">{value}</div>
  ...
  return (
    <div className="relative h-full overflow-y-auto bg-background text-foreground">
      ...
      <div className="space-y-4 px-5 py-5">
        ...
        <div>
          <h1 className="font-display text-3xl leading-tight text-moss">{plant.popular}</h1>
        ...
        <div className="rounded-2xl border border-xp/40 bg-xp/12 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="font-display text-xl leading-none text-moss">+10 XP</p>
...
function MissionUpdated({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center bg-background px-6 text-center text-foreground">
      ...
      <h2 className="mt-5 font-display text-3xl text-moss">Missão atualizada!</h2>
      ...
      <div className="mt-6 w-full max-w-xs">
        ...
        <div className="mt-1.5 h-2.5 overflow-hidden rounded-full bg-secondary">
          <div className="h-full w-[40%] rounded-full bg-gradient-to-r from-moss to-sage" />
        </div>
      </div>
